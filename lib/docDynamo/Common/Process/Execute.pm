...
####################################################################################################################################
package docDynamo::Common::Process::Execute;
...
use docDynamo::Common::Io::Handle;
use docDynamo::Common::Io::Buffered;
use docDynamo::Common::Log;
use docDynamo::Common::Wait;
...
sub begin
{
    ...
    $self->{oIo} = new docDynamo::Common::Io::Buffered(new docDynamo::Common::Io::Handle('exec test', $self->{hOut}), 0, 65536);

    # Create buffered error object
    $self->{oIoError} = new docDynamo::Common::Io::Buffered(
        new docDynamo::Common::Io::Handle('exec test', $self->{hError}), 0, 65536);
...
sub executeTest
{
    ...
    my $oExec = new docDynamo::Common::Process::Execute($strCommand, $oParam);
    $oExec->begin();
