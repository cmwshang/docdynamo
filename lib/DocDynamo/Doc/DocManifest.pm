...
sub new
{
    ...
    foreach my $oSource ($self->{oManifestXml}->nodeGet('source-list')->nodeList('source'))
    {
        ...
        my $strSourceType = $oSource->paramGet('type', false);  # ??? This indicates if custom
...
sub renderOutList
{
    ...
    if (defined(${$self->{oManifest}}{render}{$strType}))
    {
        # foreach my $strOrder (@{${$self->{oManifest}}{render}{$strType}{stryOrder}}) # ???
        # {
         @stryRenderOut = sort(keys(%{${$self->{oManifest}}{render}{$strType}{out}}));
            # push(@stryRenderOut, ${$self->{oManifest}}{render}{$strType}{out}{$strOrder}); # ???
        # }
    }
