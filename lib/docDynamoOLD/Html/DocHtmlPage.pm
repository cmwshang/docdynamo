...
# ??? This is what is required for --out=HTML so even though description in the dtd is not required, it is here
# ??? But maybe could make them optional
...
# ??? This needs a hard look at how to build the menus
    ...
    if ($self->{bToc})
    {
        ...
        # ??? if want a title in the TOC, then should probably have the menu name as the strContent
...
# ??? Maybe should use a variable in the manifest to defined the TOC depth
